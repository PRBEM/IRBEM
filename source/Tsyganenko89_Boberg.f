...
c***************************************************************************************************
c Copyright 1989, 1992, N. Tsyganenko
c
c This file is part of IRBEM-LIB.
c
c    IRBEM-LIB is free software: you can redistribute it and/or modify
c    it under the terms of the GNU Lesser General Public License as published by
c    the Free Software Foundation, either version 3 of the License, or
c    (at your option) any later version.
c
c    IRBEM-LIB is distributed in the hope that it will be useful,
c    but WITHOUT ANY WARRANTY; without even the implied warranty of
c    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
c    GNU Lesser General Public License for more details.
c
c    You should have received a copy of the GNU Lesser General Public License
c    along with IRBEM-LIB.  If not, see <http://www.gnu.org/licenses/>.
c
C-----------------------------------------------------------------------
        SUBROUTINE T89CBOBERG(IOPT,dst_nt,X,Y,Z,BX,BY,BZ)
...
C
C For BOBERG implmentation, Reallocate coefficients each call
C
        ID=1
        IOP=IOPT
        DO 1 I=1,30
...
C
C
C   If Kp is above 6- (5.7) apply boberg extension using Dst. 
C   If Dst not provided, uses the fitted averaged value
C   by Kp class
C   Fit of the C5 T89c using a refitted linear regression 
        IF (iopt.eq.7) THEN           
            A(5) = -13081.0 + 1034.9 * dst_nt
        ENDIF
C
        XI(1)=X
        XI(2)=Y
...
C -------------------------------------------------------------------
C
C   This is identical as Tsyganenko89.f T89 Routine
C   It's duplicated to avoid COMMON/SAVE Edge case errors between subroutines
C
C
        SUBROUTINE  T89BOBERG (ID, A, XI, F, DER)
      IMPLICIT REAL*8 (A-H,O-Z)
      IMPLICIT INTEGER*4 (I-N)
C
