...
version.fortran:
	@if [ -d $(SOURCEDIR)/.svn ] ; then \
		...
		svn info $(SOURCEDIR) | grep ^Revision | awk '{ print "        PARAMETER (FORTRAN_VERSION = ",$$2,") ! Generated by Make File" }' >> $(SOURCEDIR)/fortran_version.inc ; \
	 else \
		echo "        INTEGER*4 FORTRAN_VERSION ! generated by Make File" > $(SOURCEDIR)/fortran_version.inc ; \
...
#------------------------------------------------------------------------------
# store #(NTIME_MAX) in ntime_max.inc
#------------------------------------------------------------------------------

ntime_max:
	@echo "        INTEGER*4 NTIME_MAX ! generated by Make File" > $(SOURCEDIR)/ntime_max.inc ; \
	 echo "        PARAMETER (NTIME_MAX = " $(NTIME_MAX) ") ! Generated by Make File" >> $(SOURCEDIR)/ntime_max.inc
