...
Bgeo = repmat(nan,ntime,3);
B = repmat(nan,ntime,1);
Bgeo1 = Bgeo(1,:);
B1 = B(1);
BgeoPtr = libpointer('doublePtr',Bgeo1);
BPtr = libpointer('doublePtr',B1);
for i = 1:ntime,
    calllib('onera_desp_lib','get_field1_',kext,options,sysaxes,iyear(i),idoy(i),UT(i),x1(i),x2(i),x3(i),maginput(i,:),...
        BgeoPtr,BPtr);
    % have to do this next bit because Ptr's aren't really pointers
    Bgeo(i,:) = get(BgeoPtr,'value');
    B(i) = get(BPtr,'value');
end
